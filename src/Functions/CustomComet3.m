function CustomComet3(varargin)
%% marker
custom_marker = imread('./Data/Viking.png');
markersize = [200, 200]; %//The size of marker is expressed in axis units, NOT in pixels
...
head = line('parent',ax,'color',colors(1,:),'marker', 'p', ...
    'xdata',x(1),'ydata',y(1),'zdata',z(1),'tag','head');
   
% Choose first three colors for head, body, and tail
body = animatedline('parent',ax,'color','b','linestyle',lstyle,...
                    'MaximumNumPoints',max(1,k),'Tag','body', 'LineWidth', 2);
tail = animatedline('parent',ax,'color','c','linestyle','-',...
                    'MaximumNumPoints',1+m, 'Tag','tail', 'LineWidth', 1.5);
